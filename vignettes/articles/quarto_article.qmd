...
```{r setup}
#| message: false
# Load package if available (for local development)
# During R CMD check, the package may not be installed yet
if (requireNamespace("rpt", quietly = TRUE)) {
  library(rpt)
}
```
...
```{r}
# Numeric vector
rpt::example_function(c(5, 10, 15, 20, 25))

# Using with generated data
random_data <- runif(10, min = 0, max = 100)
rpt::example_function(random_data)
```
...
# <1>
data <- c(1, 2, 3, 4, 5) # <2>
result <- rpt::example_function(data) # <3>
print(result) # <4>
```
1. Load sample data
